...
音声合成を行う CPU スレッド数は、実行時引数または環境変数で指定できます。
両方を指定した場合は実行時引数が優先されます。
...
- 自動指定では E コアを使わず、異なる物理 P コアから論理 CPU を 1 個ずつ選び、少なくとも物理 P コアを 1 個残します。
  SMT により物理 P コアに複数の論理 CPU がある場合は物理 P コア数を、それ以外は P コアの論理 CPU 数の半分を基準にします。
  半分は小数点以下を切り捨て、全ての物理 P コアを使う数になる場合は 1 個減らします。
  利用可能な物理 P コアが 1 個だけの場合は起動エラーになるため、正数を明示的に指定してください。
- 正数を指定すると、各物理 P コアから論理 CPU を 1 個ずつ、P コアの残りの SMT の論理 CPU、E コアの順に、指定数まで実行可能な集合へ加えます。
  利用可能な論理 CPU 数を超える指定は起動エラーになります。
  この順序は集合への追加順であり、E コアを含めた後に音声合成ランタイムが P コアを優先して実行することは保証しません。

Windows の P/E コア対応は単一の Processor Group に限られ、複数の Processor Group がある P/E コア構成では起動エラーになります。
...
Windows の CPU Set API が利用できない場合や、Linux で上記の sysfs が両方ともない場合も同様です。
正数を指定した場合はその値を渡し、自動指定では論理 CPU 数の半分を小数点以下切り捨てで渡します。
ただし、論理 CPU 数を取得できない場合、または取得できた物理コア数と論理 CPU 数が異なる場合は、`0` を渡して音声合成ランタイムにスレッド数の決定を任せます。
